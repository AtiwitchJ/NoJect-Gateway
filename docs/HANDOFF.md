...
NoJect is completely set up, fully architected, tested across 4 target environments, positioned against international standards, and synced with GitHub.

### ✅ Completed Deliverables:
1. **Core Gateway (Go)**:
   - Microsecond L7 reverse proxy with dynamic routing.
   - Deterministic Fast WAF (`SQLi`, `XSS`, `CMD Injection`, `Path Traversal`).
   - Multi-Auth engine (SHA-256 API Keys with constant-time compare, JWT/OIDC RS256/ES256, HMAC-SHA256).
...
2. **AI Guard Engine (Python)**:
   ...
   - FastAPI server with `/healthz`, `/inspect/request` (request inspection), and `/inspect/response` (response inspection) endpoints.
3. **In-Process SDKs**:
...
4. **Benchmarks & Infographics**:
   ...
   - Security Protection Score Matrix — chart figures are illustrative projections (`scripts/generate_charts.py`), not measured; the actual measured TP/FP/TN/FN score comes from `tests/security_score_test.go`.
   - Multi-Model LLM Sentinel Judge Comparison (`docs/assets/sentinel_models_benchmark.png`) — illustrative projected figures (`guard-engine/sentinel_benchmark.py`); no model is invoked and no third-party product was actually measured.
   - Latency figures (0.009 ms / 9 µs overhead) — measured from the Go fast-path benchmarks (`make bench-go`), not the Python projection scripts.
...
- `make build` : Compiles Go gateway binary to `bin/noject-gateway`.
- `make run-gateway` : Runs Go gateway on port 8080.
- `make run-guard` : Runs Python AI guard engine on port 50051.
- `make test-go` : Runs Go unit and E2E security tests.
- `make test-py` : Runs Python guard-engine pytest.
- `make test-python-lib` : Runs Python library pytest via Astral `uv`.
