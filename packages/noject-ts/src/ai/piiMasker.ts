import { stripZeroWidth } from './textNormalize';
...
export class PIIMasker {
  private patterns: Array<{ type: string; regex: RegExp; replacement: string }> = [
  ...
  public mask(text: string): MaskPIIResult {
    if (!text) {
      return { maskedText: text, hasPii: false, entitiesFound: [], standardCode: 'ISO/IEC 42001 B.7.2 / OWASP LLM02:2025' };
    }

    const romanDigits: Record<string, string> = { 'Ⅰ': '1', 'Ⅱ': '2', 'Ⅲ': '3', 'Ⅳ': '4', 'Ⅴ': '5', 'Ⅵ': '6', 'Ⅶ': '7', 'Ⅷ': '8', 'Ⅸ': '9' };
    const numberWords: Record<string, string> = { zero: '0', one: '1', two: '2', three: '3', four: '4', five: '5', six: '6', seven: '7', eight: '8', nine: '9' };
    let masked = stripZeroWidth(text.replace(/[ⅠⅡⅢⅣⅤⅥⅦⅧⅨ]/g, (ch) => romanDigits[ch]).normalize('NFKC'));
    masked = masked.replace(/(?<!\w)(?:(?:zero|one|two|three|four|five|six|seven|eight|nine)[\s-]+){6,}(?:zero|one|two|three|four|five|six|seven|eight|nine)(?!\w)/gi,
      (run) => (run.match(/[A-Za-z]+/g) ?? []).map((word) => numberWords[word.toLowerCase()]).join(''));
    masked = masked.replace(/\bsk-(?=(?:[A-Za-z0-9_-]\s*){10,})(?:[A-Za-z0-9_-]\s*){10,64}/g,
